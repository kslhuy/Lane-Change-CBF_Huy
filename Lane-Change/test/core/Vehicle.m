classdef Vehicle < handle
    ...
    methods
        function self = Vehicle(vehicle_number , dynamics_flag, veh_param, state_initial, initial_input, controller, initial_lane_id, dt, lanes, direction_flag, acc_flag, scenario)
            self.vehicle_number = vehicle_number ;
            self.dynamics_flag = dynamics_flag;
            self.param = veh_param;
            self.controller = controller;
            if ~isempty(self.controller)
                self.controller.vehicle_number = vehicle_number;
            end
            % self.controller.vehicle_number = vehicle_number;
        ...
        function update(self)
            ...
            elseif self.dynamics_flag == 2
                lane_change_car_update(self); % Now same as : normal_car_update(self);
            end
        end
        ...
        function h =  plot_vehicle(self)
            ...
            %     self.draw_vehicle(self.state, (self.param.l_fc + self.param.l_rc), self.param.width, self.dynamics_flag);
            % end
            h = self.draw_vehicle(self.state, (self.param.l_fc + self.param.l_rc), self.param.width, self.dynamics_flag);
        ...
        function  [A , B] = matrix(self, state, input)
            A = [ 0 0 cos(theta);
                 0 0 sin(theta);
                 0 0 0];
        ...
        function BicycleModel_IDM(self)
            % Parameters
            a_max = 1.5; % maximum acceleration (m/s^2)
            b_max = 2.0; % maximum deceleration (m/s^2)
            v0 = 30; % desired velocity (m/s)
            T = 1.5; % safe time headway (s)
            s0 = 2; % minimum distance (m)
            a = 1.0; % acceleration exponent

            % State variables
            x = self.state(1); % position (m)
            y = self.state(2); % lateral position (m)
            theta = self.state(3); % orientation (rad)
            v = self.state(4); % velocity (m/s)
            delta = self.state(5); % steering angle (rad)

            % IDM longitudinal control
            s = self.get_distance_to_lead_vehicle(); % distance to lead vehicle (m)
            delta_v = self.get_relative_velocity_to_lead_vehicle(); % relative velocity to lead vehicle (m/s)
            s_star = s0 + max(0, v * T + (v * delta_v) / (2 * sqrt(a_max * b_max))); % desired gap (m)
            a_idm = a_max * (1 - (v / v0)^a - (s_star / s)^2); % acceleration (m/s^2)

            % Bicycle model dynamics
            L = self.param.wheelbase; % wheelbase (m)
            beta = atan((L / 2) * tan(delta) / L); % slip angle (rad)
            x_dot = v * cos(theta + beta); % longitudinal velocity (m/s)
            y_dot = v * sin(theta + beta); % lateral velocity (m/s)
            theta_dot = (v / L) * tan(delta); % angular velocity (rad/s)
            v_dot = a_idm; % longitudinal acceleration (m/s^2)

            % Update state
            self.state(1) = x + x_dot * self.dt;
            self.state(2) = y + y_dot * self.dt;
            self.state(3) = theta + theta_dot * self.dt;
            self.state(4) = v + v_dot * self.dt;
        end
        ...
        function[h] = draw_vehicle(self, state, L, H, dynamics_flag)
            theta = state(3);
            center1 = state(1);
            center2 = state(2);
            ...
            h = patch('Vertices', [x_coor; y_coor]', 'Faces', [1, 2, 3, 4], 'Edgecolor', color, 'Facecolor', color, 'Linewidth', 1.2);
            axis equal;
        end
    end
end