...
\end{equation}
Let $G_{x,j}(T_c)$ be an upper bound, in the chosen state norm, on the nonlinear bicycle prediction gain over any admissible $T_c$-step block. Because the one-step bicycle prediction is Lipschitz with constant $L_{x,j}$ in this norm on the compact admissible region, the $T_c$-step prediction gain satisfies
\begin{equation}
    G_{x,j}(T_c)\le L_{x,j}^{T_c}.
\end{equation}
Since $L_{x,j}$ is scalar, it factors out of the matrix product. In particular, for the componentwise error-size vector
\begin{equation}
E^{(j)}(t)=\col{\|e_1^{(j)}(t)\|_*,\ldots,\|e_N^{(j)}(t)\|_*},
\end{equation}
the homogeneous recursion satisfies, componentwise,
\begin{align}
E^{(j)}(t+T_c)
&\le L_{x,j}^{T_c}
...
\end{align}
This explains why the prediction gain and the trust-mixing product separate even though prediction and mixing are interleaved in time. If
\begin{equation}
...
\begin{enumerate}
    ...
    \item Choose the local observer gain $F_j(t)$ so that the zero-input local error recursion is uniformly locally exponentially stable. With full-state local measurements, verify the resulting LTV local-error recursion along the considered trajectory, for example using the parameter-dependent Lyapunov inequality in Lemma~\ref{lem_local_iss}; frozen-time eigenvalue placement alone is not sufficient.
...
\label{eq_raw_eDO}
\end{align}
where $\hat x_{i,c}^{(j)}$ is the trust-weighted correction in~\eqref{eq_consensus_state}; time arguments on its right-hand side are omitted for readability. The analysis is local on a compact operating region $\Omega_x\times\Omega_u$ with bounded speed and inputs, no yaw-chart discontinuity, and $|\delta|\le\delta_{\max}<\pi/2$. On this region there are finite $L_{x,j},L_{u,j}>0$ such that
\begin{equation}
\begin{aligned}
...
By Lemma~\ref{lem_weight_admissible}, $M^{(j)}(t)=I_N-\bar\L^{(j)}(t)$ is entrywise nonnegative and its $i$th row sum is $1-w_{i0}^{(j)}(t)$. Nonnegativity allows the triangle inequality to pass through the weighted correction: for every host $i$,
\begin{align}
...
\end{align}
where $M^{(j)}=I_N-\bar\L^{(j)}$, $w_0^{(j)}$ stacks the anchor weights, and $\rho^{(j)}$ is zero in the no-wrap Euclidean chart (or a bounded second-order retraction remainder).
...
\end{align}
where $\bar A_j$ is finite on the compact region. Thus $\|d_{\mathrm{att}}^{(j)}\|\le\bar d_{\mathrm{att}}$ for some finite $\bar d_{\mathrm{att}}$; after rejection, the corrupted channel's term in~\eqref{eq_attack_input} is zero.
...
There are finite $\bar d_0$, $\bar d_{\DO}$, and $\bar d_{\mathrm{att}}$ such that $\|d_0^{(j)}\|\le\bar d_0$, $\|d_{\DO}^{(j)}\|\le\bar d_{\DO}$, and $\|d_{\mathrm{att}}^{(j)}\|\le\bar d_{\mathrm{att}}$. Hence $\|\tilde d_{\DO}^{(j)}\|\le\bar{\tilde d}_{\DO}:=\bar d_{\DO}+\bar d_{\mathrm{att}}$.
\end{assumption}
...
\end{lemma}
\begin{proof}
Uniform local exponential stability of the zero-input recursion gives a transition bound $c_{\ell}\lambda_{\ell}^{t-k}$. Unrolling the perturbed local recursion and using Assumption~\ref{assum_distur} gives the geometric ISS estimate~\eqref{eq_local_iss_bound}. The Lyapunov inequality~\eqref{eq_ltv_lyapunov} establishes uniform exponential stability of the LTV linearization; the uniformly quadratic higher-order remainder then gives uniform local exponential stability of the nonlinear zero-input recursion on a sufficiently small invariant neighborhood.
\end{proof}
...
\end{lemma}
\begin{proof}
Use the componentwise error-size vector $E^{(j)}(t)$ defined before~\eqref{eq_componentwise_block_bound}. Equations~\eqref{eq_nonlinear_lipschitz} and~\eqref{eq_rowwise_mixing_bound}, together with the anchor and disturbance terms, give a componentwise recursion of the form
\begin{equation}
E^{(j)}(t+1)
\le L_{x,j}M^{(j)}(t)E^{(j)}(t)+b^{(j)}(t),
...
This section reports three validation layers. The numerical campaign uses a five-vehicle platoon and 15 saved runs over local, global, and simultaneous local--global attacks to evaluate RMSE, trust response, source suppression, and the dominant attack cases. The QCar/LIMO validation then checks rollback under real packet timing and uses Case~2 trajectories to show closed-loop path effects. Finally, the control check uses trust as a gate between CACC and ACC and verifies positive projected spacing during the attack interval.

\subsection{Numerical Simulation Setup}

The numerical simulation uses five vehicles with all-to-all V2V communication; each vehicle runs the observer, and small modeling variations are treated as uncertainties. The main parameters are $N=5$, $T_s=0.01$~s, nominal geometry $l_f=1.11$~m and $l_r=1.74$~m, giving $L_w=2.85$~m, local-trust product-fusion exponents $(\omega_p,\omega_v,\omega_\psi,\omega_u)=(2.0,3.0,1.0,1.0)$ for position/distance, velocity, heading, and acceleration/input consistency, single-rating aging weight $\beta_{\text{trust}}=0.45$, class confidence $C_{\mathrm{tr}}=0.2$, $q=5$ trust classes, heading tolerance $\psi_{\mathrm{th}}=0.35$~rad, and trust threshold $\theta_{\min}=0.5$. The observer-weight module uses base gains $w_{0,\mathrm{base}}=0.4$ and $w_{\mathrm{self,base}}=0.2$, with nominal neighbor budget $0.4$ and per-neighbor cap $0.4$.

For the direct-anchor case of Theorem~\ref{thm_observer}, the contraction check uses the scaled infinity norm $\|z\|_* = \|D_x z\|_\infty$, $D_x=\mathrm{diag}(0.01,0.01,1,0.2)$, to account for mixed state units. On $v\le33$~m/s and $|\delta|\le0.35$~rad, the induced one-step bicycle gain is $L_{x,\max}=1.066$. With accepted anchor mass $\underline w_0=\eta_0\theta_{\min}=0.20$, Lemma~\ref{lem_anchor_contraction} gives $\alpha=L_{x,\max}(1-\underline w_0)=0.853<1$ for the all-to-all accepted-anchor topology. If a target anchor is rejected, no reconstruction from communication alone is claimed; the implementation holds or marks the estimate as temporarily unreachable until an accepted anchored path returns.

\subsection{Evaluation Metrics}
The validation reports estimation accuracy, trust response, source-weight suppression, severe residual-error cases, rollback behavior, and projected spacing preservation.
...
The main numerical metrics are channel-wise RMSE, trust detection/recovery delay, attacker-source influence, and source suppression delay. The saved workbooks report longitudinal position $X$ (labeled distance in the MATLAB output), yaw $\psi$, speed $v$, and acceleration $a$. For reported channel $q\in\{X,\psi,v,a\}$ and vehicle $i$,
\begin{equation}
\operatorname{RMSE}_{q,i}=
\sqrt{\frac{1}{|\mathcal{T}|}\sum_{t\in\mathcal{T}}
\left(q_i(t)-\hat q_i(t)\right)^2}.
\end{equation}
For yaw, the error is $\mathrm{wrap}_{\pi}(\psi_i-\hat\psi_i)$. Saved-run attack-window RMSE is computed over $[10,15]$~s for non-attacker entries. Detection delay is measured from $10$~s until the mean trust assigned to the attacker falls below $\theta_{\mathrm{op}}=0.70$, and recovery delay is measured from $15$~s until the same trace remains above $\theta_{\mathrm{op}}$ for $0.5$~s. Attacker-source influence is the neighbor or global-source weight assigned to the attacker, excluding direct self-weight; suppression delay is the first attack-window time at which this influence remains below $10\%$ of its pre-attack mean for $0.5$~s. The raw combined RMSE used for ranking in the saved analysis is
\begin{equation}
\begin{aligned}
\operatorname{RMSE}_{\mathrm{raw},i}
&=\operatorname{RMSE}_{X,i}
+\operatorname{RMSE}_{v,i}
+\operatorname{RMSE}_{a,i}.
\end{aligned}
\end{equation}
Because this sum mixes physical units, it is used only as a relative severity index within the same setup; yaw is reported separately and is not included in this combined ranking score. False-positive trust rejection is computed separately as the fraction of benign non-self source trust samples with $\VT_{i,l}<\theta_{\min}$ over the reported window. The saved data do not contain the full post-attack state/input histories needed for estimation-error recovery, so recovery is reported only for trust and source-weight traces. The closed-loop check is limited to positive path-projected spacing during the attack interval.
...
The numerical campaign is organized around five bicycle-state attack cases, summarized in Table~\ref{tab:attack_mix}. Each attack is active on the interval $[10,15]$~s. The campaign contains 15 saved run files: local-channel corruption, global-channel corruption, and simultaneous local--global corruption, each crossed with attacker identities V1--V5. Within each file, the five attack IDs are evaluated as separate cases. The lead scenario, initial platoon state, and fixed parameter spread are reused across runs; intermittent faults, packet drops, and measurement noise use the random draws stored in each saved run, so the results should be interpreted as one saved stochastic campaign rather than Monte Carlo averages. Unless stated otherwise, the summaries below aggregate over state channels, attacker locations, attack cases, corruption channels, and non-attacker target vehicles.
...
\midrule
1 & Bias & Longitudinal position $X$ & bias = $-5$~m \\
2 & Faulty & Longitudinal position $X$ & std. = 10~m, $p = 0.5$ \\
...
The trust layer detected every injected attack. On the $T_s=0.01$~s grid, the mean detection delay is $0.0146$~s, with case-level averages from $0.0075$~s to $0.0500$~s. Mean attacker trust decreases from $0.822$ before attack to $0.207$ during attack, a $0.615$ drop or about $74.8\%$, and returns above $\theta_{\mathrm{op}}=0.70$ after $0.324$~s on average. At the observer input, mean attacker-source influence decreases from $0.1386$ to $0.0095$, a $93.1\%$ reduction, and is exactly zero in $88.5\%$ of attack-window samples. False-positive trust rejection, measured over benign non-self source samples against $\theta_{\min}=0.5$, averages $7.94\%$ during the attack window; the corresponding local, combined local--global, and global rates are $11.88\%$, $11.86\%$, and $0.08\%$.

Table~\ref{tab:attack_mode_rmse_summary} compares the three corruption channels. Global-channel corruption is most damaging, with mean raw combined RMSE $0.1666$, about $22.0\%$ above local-only corruption and $18.7\%$ above simultaneous local--global corruption.
...
The both-channel case remains closer to local-only than to global-only corruption because the added local inconsistency makes the compromised source easier to classify and quarantine, limiting its effective observer weight.

Table~\ref{tab:case_ranking} ranks the five attack cases by mean raw combined RMSE over all channels and attackers. Case~2, the intermittent $10$~m longitudinal-position fault, is most severe with mean raw combined RMSE $0.1737$. The largest single vehicle-level impact also occurs in Case~2: global corruption from attacker V5 drives target V4 to raw combined RMSE $0.3175$.
...
\midrule
2 & X Faulty $10$~m        & $0.1737$ & $0.301$ \\
4 & V Faulty $2.5$~m/s     & $0.1481$ & $0.313$ \\
5 & DoS                    & $0.1418$ & $0.374$ \\
3 & V Bias $-2.5$~m/s      & $0.1403$ & $0.034$ \\
1 & X Bias $-5$~m          & $0.1355$ & $0.013$ \\
\bottomrule
\end{tabular}
\end{table}

Fault-type attacks, especially the $10$~m longitudinal-position fault, create the largest estimation degradation. Bias attacks produce smaller RMSE but stronger trust suppression because their persistent offsets are easier to recognize. Thus, low attacker trust confirms detection, whereas RMSE measures residual damage before and during weight suppression. The worst cell is used only as a campaign maximum, not as evidence that one attacker location is intrinsically more harmful; the robust conclusion is that global longitudinal-position-fault corruption dominates residual estimation error.

Table~\ref{tab:source_suppression_by_channel} reports attacker-as-source influence, excluding direct self-weight, so the values measure how much the corrupted vehicle can affect other distributed observers.
...
The both-channel case has the smallest residual source influence, with average attack-window source weight $0.0008$ and zero-weight rate $99.1\%$, explaining why its RMSE remains close to local-only corruption. Global-only corruption is more damaging because it can remain partially trusted longer, with a larger attack-window source weight and slower suppression delay.

Overall, the trust layer identifies all saved attacks with mean delay $0.0146$~s, reduces attacker-source influence by $93.1\%$ on average, and assigns zero source weight in most attack-window samples. The remaining limitation is residual damage from global longitudinal-position-fault corruption before the source weight has fully collapsed, especially in Case~2; this condition should guide future tuning of trust thresholds, source suppression, and rollback activation.
...
At vehicle level, both platforms share the normalized $(X,Y,\psi,v)$ interface. The longitudinal branch projects position onto the road tangent and applies cooperative cruise control, while the lateral branch uses cross-track and heading errors for steering.
...
To isolate rollback and anchoring, Fig.~\ref{fig:rollback_anchoring} compares three runs under the same attack configuration: trust-based estimation without rollback, with rollback, and with rollback plus relative pose anchoring. The time axis is shifted so $t=0$ is attack onset. The target is V0, and the rejection signal is the direct attacked-source weight $w_{0}^{(0)}$ for the attacked direct/local channel.
\begin{figure}[!t]
    \centering
    \includegraphics[width=\linewidth]{img/rollback_anchoring.eps}
    \caption{Case 1 local attack on QCar/LIMO: rollback and relative-pose anchoring under real-vehicle delay.}
    \label{fig:rollback_anchoring}
\end{figure}

Figure~\ref{fig:rollback_anchoring} shows that trust rejection alone is not sufficient once corrupted data have already entered the observer. Without rollback, the direct attacked-source weight drops, but the contaminated state remains and the attack-window position RMSE reaches $3.77$~m with peak error $3.87$~m. Rollback replays the buffered observer updates after rejection; in this run, the rollback event occurs $0.058$~s after attack onset and reduces the instantaneous post-prediction error from $1.97$~m to $0.032$~m, giving attack-window RMSE $0.168$~m.

Rollback plus anchoring gives the smallest residual error because the relative pose anchor limits drift during the rejected-source interval. The attack-window RMSE decreases to $0.0218$~m, with maximum error $0.0243$~m. The three curves come from separate runs, so small timing differences in trust flags, rollback triggers, and direct-weight recovery should be interpreted as run-to-run timing variation rather than sample-synchronized traces.

\subsubsection{Closed-Loop Trajectory Diagnostics}

The same platform logs show how attacked estimates affect the driven path when supplied to the controller. Figures~\ref{fig:XY_case2_local} and~\ref{fig:XY_case2_global} plot Case~2 XY trajectories under local- and global-channel attacks, so they are closed-loop diagnostics rather than estimator-only plots.

\begin{figure}[!t]
    \centering
    \includegraphics[width=\linewidth]{img/XY_case2_local.eps}
    \caption{XY trajectory under Case~2 local attack.}
...
\begin{figure}[!t]
    ...
    \caption{XY trajectory under Case~2 global attack.}
    \label{fig:XY_case2_global}
\end{figure}

In both Case~2 attacks, the corrupted source's trust and fusion weight decrease, shifting authority toward the self estimate and validated neighbors. Local attacks mainly reduce the direct/local component, whereas global attacks reduce the global-consistency component; the source-weight statistics in Table~\ref{tab:attack_mode_rmse_summary} quantify the resulting suppression.
...
\end{align}
where $e_{s,ij}$, $e_{v,ij}$, and $e_{\psi,ij}$ are the corresponding relative errors and $d_{i,j}=(i-j)(d+h\hat v_0^{(i)})$ for a homogeneous platoon. The trust-fused, filtered command is
\begin{align}
u_{\mathrm{target},i}
...
\begin{figure}[!t]
    ...
    \caption{Controller fusion spacing.}
    \label{fig:controller_fusion}
\end{figure}
\begin{table}[!t]
\centering
\caption{Attack-Window Closed-Loop Metrics for the Platform V0 Case~2 Attack}
\label{tab:closed_loop_control_metrics}
\scriptsize
\begin{tabular}{cccccc}
\toprule
Host & $\Delta t_{\mathrm{det}}$ [s] & $\bar{\gamma}_{\mathrm{att}}$ & $\bar w_{\mathrm{src}}(V0)$ & $s_{\min}$ [m] & Gap RMSE [m] \\
\midrule
...
Table~\ref{tab:closed_loop_control_metrics} summarizes two follower-host logs under the same V0 attack schedule. In both, the attacked V0 source weight falls below 0.05 on average within about 0.1~s. V1 shifts strongly toward local ACC because it directly follows V0, whereas V2 keeps more cooperation because its direct predecessor V1 remains consistent and V0 is already suppressed at the observer input. Both hosts preserve positive projected spacing: the minimum gaps are 0.681~m and 0.637~m, above the 0.30~m margin. Thus, the reported gap RMSE reflects imperfect CACC tracking under attack, not loss of longitudinal collision clearance.
