...
Fault-type attacks, especially the $10$~m position fault, create the largest estimation degradation. Bias attacks produce smaller RMSE but stronger trust suppression because their persistent offsets are easier to recognize. Thus, low attacker trust confirms detection, whereas RMSE measures residual damage before and during weight suppression. The worst cell is used only as a campaign maximum, not as evidence that one attacker location is intrinsically more harmful; the robust conclusion is that global position-fault corruption dominates residual estimation error.
...
Overall, the trust layer identifies all saved attacks with mean delay $0.0146$~s, reduces attacker-source influence by $93.1\%$ on average, and assigns zero source weight in most attack-window samples. The remaining limitation is residual damage from global position-fault corruption before the source weight has fully collapsed, especially in Case~2; this condition should guide future tuning of trust thresholds, source suppression, and rollback activation.
