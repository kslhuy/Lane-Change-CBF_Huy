...
is_know_data_not_nearby = false ; % just for test purpose, Use that we have better Trust score , meaning that we know the data all of the other vehicles
...
for l = 1:length(lead_senarios)
    ...
    global_acc_err = mean(all_global_acc_errors,2);


    % Store the errors for plotting
    mean_errors(:,:,l) = [global_dist_err ,global_theta_err, global_vel_err,global_acc_err];
end
