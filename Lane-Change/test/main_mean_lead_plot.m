...
lead_senarios = ["constant", "Acceleration", "Deceleration"];
% mean_errors = [];
number_lead_senarios = length(lead_senarios);
number_vehicles_in_platoon = 4; % Number of vehicles in the platoon
number_vehicles_to_evaluate = 3; % Number of metrics to evaluate (distance, orientation, velocity)

mean_errors = zeros(number_vehicles_in_platoon,number_vehicles_to_evaluate, number_lead_senarios);  % 4 vehicles × 3 metrics × 3 scenarios
...
for l = 1:length(lead_senarios)
    ...
    global_dist_err = mean(all_global_dist_errors,2);
    global_theta_err = mean(all_global_theta_errors,2);
    global_vel_err = mean(all_global_vel_errors,2);



    % Run the simulation and get mean errors
    % [dist_err, vel_err] = mean_simulation_lead(lead_senario);

    % Store the errors for plotting
    mean_errors(:,:,l) = [global_dist_err ,global_theta_err, global_vel_err];
end
